 ...
 * * @return >0 if message was processed, 0 if incomplete, -1 on
...
int handle_message(int sender_fd, char *buffer, ssize_t len, int listen_fd) {
  ...
  // Debug print before processing the next potential message in the buffer
  printf("handle_message processed type %u, length %zu. Remaining data in "
         "buffer: %zu\n",
    ...
    for (int i = 0; i < num_clients; i++) {
      ...
      if (FD_ISSET(client_fd, &read_fds)) {
        ...
        while ((process_result =
                    handle_message(client_fd, client_buffers[i],
                                   client_data_len[i], listen_fd)) > 0) {
        ...
        if (process_result == -2) {
          ...
          // Client disconnect during message handling (should be handled by
          // bytes_read <= 0)
        }
      }
    }
